...
router.post('/search', async (req, res) => {
    ...
    */
    ...
    let products = await conn.request()
                            ...
                            .query("SELECT products.title, products.price, products.quantity, products.description, products.image, products.id, products.short_desc, products.cat, products.brand, products.sizes, products.color FROM products WHERE products.title = CASE WHEN @productModel !='' THEN @productModel ELSE products.title END AND products.cat = CASE WHEN @productCat !='' THEN @productCat ELSE products.cat END AND products.price LIKE ''%'' AND products.brand = CASE WHEN @productBrand !='' THEN @productBrand ELSE products.brand END AND products.sizes LIKE ''%@productSize%'' AND products.color LIKE ''%@productColor%''");
