...
let conn = new Mysqli({
    Host: "shopdex.database.windows.net", // IP/domain name 
    post: 3306, // port, default 3306 
    user: "rdonino", // username 
    passwd: "231295Don*", // password 
    db: "shopdex"
});
*/
...
module.exports = {
    ...
    isPasswordAndUserMatch: async (req, res, next) => {
        ...
        user = await conn.request().input('email', mssql.VarChar, myEmail)
                                        .query("select * from dbo.users where email = @email OR username = @email");//db.table('users').filter({$or:[{ email : myEmail },{ username : myEmail }]}).get();
